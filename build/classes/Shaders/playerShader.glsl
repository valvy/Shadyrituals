...
/*
	...
	The goal is then to find the global minimum of that given problem.  The approach
	I am taking here is to stepping through the curve's parameter and trying to find a coarse
	global minimum. Then i use the bisection method on the derivative of the distance to find
	the exact minimum.
...
vec3 tone(vec3 toneColor, float gamma)
{
	float white = 2.;
	float luma = dot(toneColor, vec3(0.2126, 0.7152, 0.0722));
	float toneMappedLuma = luma * (1. + luma / (white*white)) / (1. + luma);
	toneColor *= toneMappedLuma / luma;
	toneColor = pow(toneColor, vec3(1. / gamma));
	return toneColor;
}
...
void main(void)
{
    ...
    col *= (texture2D(iChannel0,p*0.00025*iResolution.x+fract(time)*vec2(51,100)).r-0.5)*.24+1.;
    col = mix(col,tone(col,1.7),0.5);
    
    #elif COLOR_TYPE == 2
    vec3 col = vec3(5.,1.5,.5)*log(rz+1.05);
    col = tone(col,8.5);
    col *= (texture2D(iChannel0,p*0.00017*iResolution.x+fract(time)*vec2(51,100)).r-0.5)*.12+1.;
    
    #else
    rz = pow(rz*50.,.7);
    vec3 col = vec3(.6,.2,1.)/(rz+1.5)*9.;
    col *= (texture2D(iChannel0,p*0.00025*iResolution.x+fract(time)*vec2(51,100)).r-0.5)*.45+1.;
    col = tone(col,1.6);
    #endif
